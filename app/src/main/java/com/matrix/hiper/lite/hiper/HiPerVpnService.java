...
public class HiPerVpnService extends VpnService {
    ...
    private static moblieVlan.HiPer hiper = null;
    ...
    private void startVpn() {
        ...
        try {
            ipNet = moblieVlan.MoblieVlan.parseCIDR(site.getCert().getCert().getDetails().getIps().get(0));
        } catch (Exception e) {
        ...
        for (Sites.UnsafeRoute unsafeRoute : site.getUnsafeRoutes()) {
            try {
                CIDR cidr = moblieVlan.MoblieVlan.parseCIDR(unsafeRoute.getRoute());
        ...
        new Thread(() -> {
            try {
                hiper = moblieVlan.MoblieVlan.newHiPer(site.getConfig(), site.getKey(this), site.getLogFile(), vpnInterface.getFd());
                handler.post(() -> {
