...
public class HiPerVpnService extends VpnService {
    ...
    private static moblie.Bulk hiper = null;
    ...
    private void startVpn() {
        ...
        try {
            ipNet = moblie.Moblie.parseCIDR(site.getCert().getCert().getDetails().getIps().get(0));
        } catch (Exception e) {
        ...
        for (Sites.UnsafeRoute unsafeRoute : site.getUnsafeRoutes()) {
            try {
                CIDR cidr = moblie.Moblie.parseCIDR(unsafeRoute.getRoute());
        ...
        new Thread(() -> {
            try {
                hiper = moblie.Moblie.newBulk(site.getConfig(), site.getKey(this), site.getLogFile(), vpnInterface.getFd());
                handler.post(() -> {
